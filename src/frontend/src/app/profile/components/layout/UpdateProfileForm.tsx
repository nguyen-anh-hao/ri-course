...
const UpdateProfileForm: React.FC = () => {
    ...
    const [createdAt, setCreatedAt] = useState<string | null>(null); // Ngày tạo tài khoản
    const [updatedAt, setUpdatedAt] = useState<string | null>(null); // Ngày cập nhật tài khoản
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                setCreatedAt(response.data.createdAt ? new Date(response.data.createdAt).toLocaleDateString() : null);
                setUpdatedAt(response.data.updatedAt ? new Date(response.data.updatedAt).toLocaleDateString() : null);
            } catch (error) {
            ...
            <Box component='form' onSubmit={(e) => { e.preventDefault(); handleUpdateProfile(); }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Typography variant="body1" color="textSecondary">
                        <strong>Ngày tạo tài khoản: </strong>{createdAt || 'Chưa có thông tin'}
                    </Typography>
                    <Typography variant="body1" color="textSecondary">
                        <strong>Ngày cập nhật: </strong>{updatedAt || 'Chưa có thông tin'}
                    </Typography>
                </Box>
